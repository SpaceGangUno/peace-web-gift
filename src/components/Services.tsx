...
const Services = () => {
  ...
  return <section id="services" className="section section-alt md:py-24 lg:py-32 py-[51px]">
      <div className="container-custom max-w-7xl">
        ...
        {isMobile ? <Carousel className="w-full max-w-sm mx-auto services-carousel mt-10" opts={{
        loop: true,
        slidesToScroll: 1,
        dragFree: false,
        // Make sliding more controlled
        active: true,
        // Enable active state
        align: "center",
        ...
        watchDrag: true // Ensure drag events are monitored
            ...
            <CarouselContent className="-ml-2 md:ml-0"> {/* Adjust left margin */}
              {services.map((service, index) => <CarouselItem key={index} className="pl-2 md:pl-4">
          ...
          </Carousel> : <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-16">
            {services.map((service, index) => <ServiceCard key={index} service={service} animationDelay={index * 0.15} />)}
          </div>}
...
}) => <div className="group animate-fade-in-up h-full" style={{
  animationDelay: `${animationDelay}s`
}}>
    <Card className={`hover-card h-full p-6 border-2 border-wasabi/20 hover:border-gold transition-all duration-300 hover:shadow-lg rounded-xl flex flex-col relative overflow-hidden ${service.backgroundImage ? 'text-white' : 'bg-white/80 backdrop-blur-sm'}`}>
      {service.backgroundImage && <div className="absolute inset-0 z-0">
      ...
      <div className={`${service.backgroundImage ? 'relative z-10 h-full flex flex-col' : 'h-full flex flex-col'}`}>
        <div className="mb-4 flex items-center justify-between">
          {service.comingSoon && <span className="text-xs font-sans bg-magenta-500 text-white px-3 py-1.5 rounded-full font-medium shadow-md border border-white/30 animate-pulse">
              Coming Soon
            </span>}
        </div>
        ...
        {service.showConsultButton && <div className="mt-auto pt-4">
            <Link href="/schedule">
              <Button variant="outline" size="sm" className={`w-full ${service.backgroundImage ? 'border-gold bg-gold/30 hover:bg-gold/50 text-white' : 'border-gold hover:bg-gold/20 text-emerald-green'} 
